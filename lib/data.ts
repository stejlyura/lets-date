...
const getZodiac = (dobStr: string): string => {
...
const occupations: string[] = [
  "UI/UX Designer", "Software Engineer", "Creative Director", "Architect", "Photographer",
  "Fashion Stylist", "Digital Marketer", "Veterinarian", "Yoga Instructor", "Event Producer",
  "Interior Designer", "Product Manager", "Journalist", "Biochemist", "Pastry Chef"
];

export const PROFILES: Profile[] = profilesArray.map((p, index) => {
  const ext = p.id === "a1c9e8f7-6b5d-4a3c-8f2e-9d8c7b6a5f4e" ? ".jpeg" : ".webp";
  return {
    ...p,
    photoUrl: `/${p.id}${ext}`,
    zodiacSign: getZodiac(p.dateOfBirth),
    occupation: occupations[index % occupations.length],
    distanceKm: Math.floor(2 + (index * 1.7) % 25),
    compatibilityScore: Math.floor(88 + (index * 3) % 12),
    instagramHandle: `@${p.name.toLowerCase()}_${p.city.toLowerCase().replace(/[^a-z]/g, "")}`,
    likedYou: true,
  };
});
